...
// ─── Task Import ─────────────────────────────────────────────
const importPanels = {
  todoist:      document.getElementById('todoistPanel'),
  mstodo:       document.getElementById('msTodoPanel'),
  googletasks:  document.getElementById('googleTasksPanel'),
};

function toggleImportPanel(service) {
  Object.entries(importPanels).forEach(([key, panel]) => {
    const btn = document.getElementById(
      key === 'todoist' ? 'importTodoistBtn' :
      key === 'mstodo'  ? 'importMsTodoBtn'  : 'importGoogleTasksBtn'
    );
    if (key === service) {
...
document.getElementById('importTodoistBtn').addEventListener('click', () => toggleImportPanel('todoist'));
document.getElementById('importMsTodoBtn').addEventListener('click', () => toggleImportPanel('mstodo'));
document.getElementById('importGoogleTasksBtn').addEventListener('click', () => toggleImportPanel('googletasks'));

// Render imported tasks as clickable suggestions
function renderImportedList(tasks, listId) {
  const ul = document.getElementById(listId);
  ul.innerHTML = '';
  if (tasks.length === 0) {
    ul.innerHTML = '<li style="color:#9ca3af;cursor:default">No tasks found</li>';
    return;
  }
  tasks.forEach(text => {
    const li = document.createElement('li');
...
// Restore saved token
const savedTodoistToken = localStorage.getItem('todoistToken');
if (savedTodoistToken) document.getElementById('todoistTokenInput').value = savedTodoistToken;

document.getElementById('todoistFetchBtn').addEventListener('click', async () => {
  const btn   = document.getElementById('todoistFetchBtn');
  const token = document.getElementById('todoistTokenInput').value.trim();
  if (!token) {
    todoistStatus.textContent = 'Please enter your API token.';
    todoistStatus.className = 'import-status error';
    return;
  }
  btn.textContent = 'Loading…';
  btn.disabled = true;
  todoistStatus.textContent = '';
  todoistStatus.className = 'import-status';
  ...
  try {
    ...
    renderImportedList(tasks.map(t => t.content), 'todoistResultList');
    todoistStatus.textContent = `✓ ${tasks.length} task${tasks.length !== 1 ? 's' : ''} loaded`;
    todoistStatus.className = 'import-status success';
  } catch (err) {
    todoistStatus.textContent = `✗ ${err.message}`;
    todoistStatus.className = 'import-status error';
  } finally {
...
// ── Microsoft To-Do (MSAL.js) ─────────────────────────────────
document.getElementById('msTodoSignInBtn').addEventListener('click', async () => {
  const clientId = document.getElementById('msTodoClientId').value.trim();
  if (!clientId) {
    alert('Please enter your Azure App Client ID.\n\nCreate one free at portal.azure.com → Azure AD → App Registrations.\nAdd redirect URI: ' + location.origin + '\nAdd permission: Tasks.Read (Microsoft Graph)');
    return;
  }
  try {
    ...
    if (!window.msal) {
      await new Promise((resolve, reject) => {
        ...
        s.src = 'https://alcdn.msauth.net/browser/2.38.2/js/msal-browser.min.js';
    ...
    const result = await msalApp.loginPopup({ scopes: ['Tasks.Read'] });
    const tokenRes = await msalApp.acquireTokenSilent({
      scopes: ['https://graph.microsoft.com/Tasks.Read'],
    ...
    for (const list of lists) {
      ...
      const items = (await tasksRes.json()).value || [];
      allTasks.push(...items.map(t => t.title));
    }
    renderImportedList(allTasks, 'msTodoResultList');
  } catch (err) {
    alert(`Microsoft To-Do error: ${err.message}`);
  }
});

// ── Google Tasks (Google Identity Services) ───────────────────
document.getElementById('googleSignInBtn').addEventListener('click', async () => {
  const clientId = document.getElementById('googleClientId').value.trim();
  if (!clientId) {
    alert('Please enter your Google OAuth Client ID.\n\nCreate one at console.cloud.google.com → APIs & Services → Credentials.\nEnable the Google Tasks API.\nAdd authorised origin: ' + location.origin);
    return;
  }
  try {
    ...
    const tokenClient = google.accounts.oauth2.initTokenClient({
      ...
      callback: async (response) => {
        if (response.error) { alert(`Google error: ${response.error}`); return; }
        ...
        for (const list of lists) {
          ...
          const items = (await tasksRes.json()).items || [];
          allTasks.push(...items.map(t => t.title));
        }
        renderImportedList(allTasks, 'googleResultList');
      }
    });
